    ...
    db.get_cmc(settings.coinmarketcap.ticker, function(cmc) {
      lib.get_masternodecount(function(totalMnCount) {
        lib.get_masternodeonlinecount(function(activeMnCount) {
          db.get_latest_masternodestats(settings.symbol, async function(mnStats) {
	    ...
	    stats.last_price = priceBtc
            priceUsd = prices.usd;
	    cmc.price_usd = prices.usd
	    cmc.market_cap_usd = prices.market_cap_usd
	    ...
	    mnRewardsPerDay = (await getMNStats()).daily_income
            ...
            var data = {
              ...
              pricePercChange24h: prices.price_change_percentage_24h.toFixed(2),
          ...
          db.get_stats(settings.coin, function (stats) {
            db.get_latest_masternodestats(settings.symbol, function(mnStats) {
              ...
              var data = {
                ...
                mobile_app_v: 1,
                supply: stats.supply,
                last_price_btc: stats.current_price,
                last_price_usd: cmc.price_usd,
                market_cap_usd: cmc.market_cap_usd,
                market_volume_24h_usd: cmc.volume_24h_usd,
                price_perc_change_1h: cmc.price_change_24h,
                price_perc_change_24h: cmc.price_change_24h,
                price_perc_change_7d: cmc.price_change_24h * 7,
...
async function getPrices() {
  var data = await doRequest("https://api.coingecko.com/api/v3/coins/bigdata-cash")
  let prices = {
    "btc": parseFloat(JSON.parse(data).market_data.current_price.btc),
    "usd": parseFloat(JSON.parse(data).market_data.current_price.usd),
    "price_change_percentage_24h": parseFloat(JSON.parse(data).market_data.price_change_percentage_24h),
    "market_cap_change_24h": parseFloat(JSON.parse(data).market_data.market_cap_change_24h),
    "market_cap_usd": parseFloat(JSON.parse(data).market_data.market_cap.usd)
  }
  return prices;
}

async function getMNStats() {
  var data = await doRequest("https://ihostmn.com/api/v1/coinstats/public/list_coin?ticker=BDCASH")
  return JSON.parse(data).result.coin;
}
